classdef RPC < handle
    ...
    methods
        function obj = RPC(RPC,Normalize_par)
        ...
        function geo_loc = img2obj(obj,POINT)
            % map from geographic coordinates to image coordinates
            %   GEOPOINT is a class
            
            v = POINT.v;
            Yn_ = obj.p1'*v./(obj.p2'*v);
            Xn_ = obj.p3'*v./(obj.p4'*v);  
         
            Y_ = Yn_ * obj.LAT_SCALE + obj.LAT_OFF;
            X_ = Xn_ * obj.LONG_SCALE + obj.LONG_OFF;           
            Y_ = Y_'; X_ = X_';
            geo_loc = [Y_,X_];
        end
